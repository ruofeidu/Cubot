...
static final boolean FINGER_JUMP = false; 
...
int r, s=0, d=1, x, y=400, vy, wx[]={0, 0}, wy[]={0, 0}, e=1800, l=WIDTH, hs=0, v=800; 
...
void draw() { 
  ...
  for(int i=0, z=y+=vy+=1, q=(x=x-6==-1800?0:x-6); i<2; i++, fill(0), textSize(40), image(b, l/2, y)){
    ...
    if((wx[i]=wx[i]<0?(wy[i]=(int)random(WIDTH/3, v-WIDTH/3))/wy[i]*l:wx[i]-6)==l/2&&d==0) 
      hs=max(++s, hs); 
...
void jump() {
   out.playNote("C3" );
    if (d==(vy=-17)/-17) {
      //out.playNote("C4" );
      y=(wx[1]=900) + 100 - ((wy[0]=(wy[1]=wx[0]=WIDTH)-WIDTH/3)+WIDTH/3 + (x = s = d = 0)); 
    }
}
